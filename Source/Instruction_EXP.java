...
class Instruction_EXP implements Instruction_Interface
{
    Instruction_EXP() {
        ...
        try {
            if (s.Empty_1()) {
                ...
                if (base < 0.0F) {
                    stack.Pop(1);
                    stack.Replace(exp);
                    throw new Exception_EXP_Error();	// error code
                } else {
                    stack.Pop(1);
                    stack.Replace((float) java.lang.Math.pow(base,exp));
                }
                ...
                float exp = s.Get_Value2();
                        
                if (base < 0.0F) {
                    stack.Push(exp);
                    throw new Exception_EXP_Error();	// error code
                } else {
                    stack.Push((float) java.lang.Math.pow(base,exp));
                }
            ...
            if (reg.Index_To_Memory(Constants.PERROR).Get_Value() == 0.0F)
                reg.Index_To_Memory(Constants.PERROR).Set_Value(9);	// error code
            throw e;
        }
        finally {
