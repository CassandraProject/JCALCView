...
class Instruction_PRP implements Instruction_Interface
{
    Instruction_PRP() {
    ...
    public void Run (Steps_Table_Model steps, Stack_Table_Model stack, Machine reg) {
        ...
        try {
            int r = s.Get_Register1() - Constants.RO01;

            if (r < 0 || r > 7) {
                throw new Exception_Index_Error();	// error code
            } 

            int status = reg.Index_To_Memory(Constants.RI01 + r).Get_Status();
            int mask = (int) stack.Top(0);

            reg.Index_To_Memory(Constants.RO01 + r).Set_Status(status & mask);
        }
