...
class Instruction_LOG implements Instruction_Interface
{
    Instruction_LOG() {
    ...
    public void Run (Steps_Table_Model steps, Stack_Table_Model stack, Machine reg) {
        ...
        try {
            ...
            if (acc <= 0.0F) {
                throw new Exception_LOG_Error();	// error code
            } else {                
                // Since log10(x) = loge(x) / loge(10)
                acc = (float) (java.lang.Math.log(acc) / java.lang.Math.log(10.0));
            ...
            if (reg.Index_To_Memory(Constants.PERROR).Get_Value() == 0.0F)
                reg.Index_To_Memory(Constants.PERROR).Set_Value(7);	// error code
            throw e;
        }
        finally {
