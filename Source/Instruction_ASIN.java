...
class Instruction_ASIN implements Instruction_Interface
{
    Instruction_ASIN() {
    ...
    public void Run (Steps_Table_Model steps, Stack_Table_Model stack, Machine reg) {
        ...
        try {
            ...
            if (acc > 1.0F || acc < -1.0F) {
                throw new Exception_ASIN_Error();
            } else {
                acc = (float) java.lang.Math.asin(acc);
                stack.Replace(acc);
            }
        }
        catch (Exception_ASIN_Error e) {
            if (reg.Index_To_Memory(Constants.PERROR).Get_Value() == 0.0F)
                reg.Index_To_Memory(Constants.PERROR).Set_Value(2);	// error code
            throw e;
        }
        finally {
