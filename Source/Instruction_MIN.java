...
class Instruction_MIN implements Instruction_Interface
{
    Instruction_MIN() {
    ...
    public void Run (Steps_Table_Model steps, Stack_Table_Model stack, Machine reg) {
        ...
        try {
            if (s.Empty_1()) {
                ...
                for (; c > 0; --c) {
                    min = java.lang.Math.min(stack.Top(0), min);
                ...
                if (stack.Depth() > 0)
                    stack.Replace(java.lang.Math.min(acc, r));
                    
            } else if (s.Register_1() && s.Number_2()) {
                float r = reg.Index_To_Memory(s.Get_Register1()).Get_Value();
                float n = s.Get_Value2();
                            
                stack.Push(java.lang.Math.min(n, r));
                ...
                stack.Push(java.lang.Math.min(reg1, reg2));
            }
        }
