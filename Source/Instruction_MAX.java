...
class Instruction_MAX implements Instruction_Interface
{
    Instruction_MAX() {
    ...
    public void Run (Steps_Table_Model steps, Stack_Table_Model stack, Machine reg) {
        ...
        try {
            if (s.Empty_1()) {
                ...
                for (; c > 0; --c) {
                    max = java.lang.Math.max(stack.Top(0), max);
                ...
                if (stack.Depth() > 0)
                    stack.Replace(java.lang.Math.max(acc, r));
                    
            } else if (s.Register_1() && s.Number_2()) {
                float r = reg.Index_To_Memory(s.Get_Register1()).Get_Value();
                float n = s.Get_Value2();
                            
                stack.Push(java.lang.Math.max(n, r));
                ...
                stack.Push(java.lang.Math.max(reg1, reg2));
            }
        }
